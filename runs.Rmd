...
```{r universal}
sample_desc <- hash_data %>% filter(str_detect(sample, sample_id))
...
# aggr
```{r}
samples <- c("amc", "P1mc", "P2a", "A17k", "A45k", "hash18", "2hash_orig")
...
ggplot(res2, aes(x = method, y = meanF1, color = method, group = method)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(shape = sample), size = 3) +
  scale_shape_manual(values=1:length(samples)) +
  theme_classic()

ggsave("F1sx6.pdf", height = 5, width = 8)
```
